...
function ImageSearch({ searchTeext }) {
  ...
  return (
    <div className="relative w-full bg-red h-[400px]  mt-6 text-white">
      ...
      />
      <div className="px-4 sm:px-[10%] md:px-[14%] lg:px-[19%] 2xl:px-[27%] absolute w-full text-center top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 ">
        <h1 className="text-4xl tall:text-2xl font-semibold mb-4">
          Unleash your creativity with unrivaled images
        </h1>
        <h3 onClick={onClick} className="tall:text-sm font-medium">
          Add wonder to your stories with 400M+ photos, vectors, illustrations,
          and editorial images.
        </h3>
        <form
          onSubmit={onSubmit}
          className="flex my-6  justify-center relative items-center w-full m-auto"
        >
          <input
            onChange={(e) => setText(e.target.value)}
            className="w-full placeholder:text-[14px] md:placeholder:text-[16px] outline-0	p-4 pl-6 flex justify-center text-red	drop-shadow-2xl rounded h-16"
            type="text"
